load("C:/Users/lab305_PC1/Downloads/Incidence_data.RData")
library(glmnet)
library(compiler)
library(iNEXT)
library(dplyr)
...
for (i in c(1:13,15:26)) {
  dat <- incidence_dataset[[i]]$`9 鳥類`
  or_cv[i]=cv.score(dat)
  or_nos[i]=r.score(dat)
  riv <- substring(colnames(incidence_dataset[[i]][["9 鳥類"]]), 1, 6)
  ...
  if (out$t >= n) {
    add[i] <- ceiling(out$t - n)
    out$t <- ncol(dat)
    best <- best1 <-colnames(dat)
    nos <-nos1 <- r.score(dat)
    cv <-cv1 <- cv.score(dat)
    l=data.frame(total=n,choose=out$t,
                 suggestion = add[i],nos = nos, cv = cv)
    answer[["site"]]=best
    answer[["ChaoRichness"]]=cbind(ChaoRichness(dat[,best],datatype='incidence_raw'),data.frame(origin_nos=or_nos[i],origin_cv=or_cv[i]))
    answer[["information"]]=l
    results[[i]]=answer
    ...
    if (t!=1) {
      l=arrange(data.frame(total=n,choose=ceiling(out$t),
                           suggestion=ceiling(out$t)-n,nos = unlist(nos), cv = as.numeric(unlist(cv))), desc(nos), desc(cv))[1,]
      where=best[[which(cv==l$cv & nos==l$nos[1])]]
      answer[["site"]]=where
      answer[["ChaoRichness"]]=cbind(ChaoRichness(dat[,where],datatype='incidence_raw'),data.frame(origin_nos=or_nos[i],origin_cv=or_cv[i]))
      answer[["information"]]=l
      results[[i]]=answer
    }
...
for (i in miss) {
  ...
  if (choose(n, ceiling(out$t)) > 10^5) {
    ...
    count=c()
    for (k in 1:5) {
      riv2=length(unique(substring(colnames(dat[, ga[[k]]]), 1, 6)))
      count=c(count,riv2)
    }
    k=which.max(count)
    ...
    for (m in 1:nrow(num_3)) {
      dat2 <- dat[, c(ga[[k]],as.numeric(num_3[m, ]))]
    ...
    l=data.frame(total=n,choose=ncol(dat2),
                 suggestion=ncol(dat2)-n,nos = nos, cv = cv)
    ...
    count=c()
    for (k in 1:length(r_all)) {
      riv2=length(unique(substring(colnames(dat)[all[, as.numeric(names(r_all)[k])]], 1, 6)))
      count=c(count,riv2)
    }
    k=which.max(count)
    ...
    r_max <- 0
    for (m in 1:nrow(num_3)) {
      dat2 <- dat[,c(all[,as.numeric(names(r_all)[k])],as.numeric(num_3[m, ]))]
      if (r.score(dat2) > r_max) {
        best <- colnames(dat2)
        cv <- cv.score(dat2)
        nos <- r_max <- r.score(dat2)
